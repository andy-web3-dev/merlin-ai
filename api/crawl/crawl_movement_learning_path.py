...
BASE_URL = "https://developer.movementnetwork.xyz/learning-paths"
DOMAIN = "developer.movementnetwork.xyz"
OUTPUT_FILE_ALL = "../data/movement_learning_paths.txt"
OUTPUT_FOLDER = "data/movement_learning_path"
...
def is_valid_url(url):
    """
    Check if the URL is on the same domain, is within /learning-paths,
    and has no fragment (#).
    """
    parsed = urlparse(url)
    ...
    # Must start with /learning-paths
    if not parsed.path.startswith("/learning-paths"):
        return False
...
def parse_content_with_indentation(main_container):
    ...
    3. For each heading <hN>, set indent level = N-1.
       - For subsequent text (paragraphs, code, lists, etc.), use the same indent until the next heading.
    4. If the element is <pre> or <code>, wrap its text in <code> ... </code>.
    ...
    # We'll stop if we encounter <footer>, though some docs may not have it
    for element in main_container.descendants:
        ...
        # If it's a Tag (like <h2>, <p>, <pre>, etc.)
        if element.name:
            # HEADINGS
            ...
            # CODE BLOCK (<pre>, <code>)
            elif "code" in element.name or "pre" in element.name:
            ...
            # Any other tag (paragraph, div, list item, etc.)
            else:
        ...
        # If it's just a NavigableString (inline text inside a tag)
    ...
    # Join all lines, compress newlines again
    full_text = "\n".join(lines)
...
def generate_filename_from_url(url):
    ...
    E.g. https://developer.movementnetwork.xyz/learning-paths/basic => learning-paths_basic.txt
    """
    parsed = urlparse(url)
    ...
    if not path:
        path = "index"
    # e.g., "learning-paths_basic"
    return path + ".txt"


def crawl(url):
    """
    Recursively crawl valid links in /learning-paths, extract content, write it to:
...
if __name__ == "__main__":
    # 1. Create/clear the single output file
