...
import { currentModel, historyMessageLength, useConfig } from './appConfig'
...
export function useChats() {
  ...
  const handleAiPartialResponse = async (data: ChatPartResponse, chatId: number) => {
    ongoingAiMessages.value.has(chatId)
      ? await appendToAiMessage(data.message.content, chatId)
      : await startAiMessage(data.message.content, chatId)
  }
